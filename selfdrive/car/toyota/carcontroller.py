...
class CarController():
  ...
  def update(self, enabled, active, CS, frame, actuators, pcm_cancel_cmd, hud_alert,
    ...
    pcm_accel_cmd = 0 if not (enabled and CS.out.cruiseState.enabled) else clip(actuators.accel, pid_accel_limits[0], pid_accel_limits[1]) + boost
    ...
    if (frame % 3 == 0 and CS.CP.openpilotLongitudinalControl) or pcm_cancel_cmd:
      ...
      if pcm_cancel_cmd and CS.CP.carFingerprint in [CAR.LEXUS_IS, CAR.LEXUS_RC]:
        ...
        can_sends.append(create_accel_command(self.packer, pcm_accel_cmd, pcm_cancel_cmd, self.standstill_req, lead, CS.acc_type, CS.gap_adjust_cruise_tr, CS.Fast_Speed_Increments))
      else:
        can_sends.append(create_accel_command(self.packer, 0, pcm_cancel_cmd, False, lead, CS.acc_type, CS.gap_adjust_cruise_tr, CS.Fast_Speed_Increments))
    ...
    if (frame % 100 == 0 or send_ui):
      if(CS.lkasEnabled):
        can_sends.append(create_ui_command(self.packer, steer_alert, pcm_cancel_cmd, left_line, right_line, left_lane_depart, right_lane_depart, CS.lkasEnabled and not apply_steer_req, use_lta_msg, CS.lkasEnabled and apply_steer_req))
      else:
